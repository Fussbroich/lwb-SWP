...
func NewBPApp(b uint16) *bpapp {
	...
	var hintergrund Widget = NewFenster()

	var cRight Widget = NewInfoText(
		func() string { return "(c)2024 Bettina Chang, Thomas Schrader" })

	// var frames Widget = NewInfoText(
	// 	func() string { return fmt.Sprintf("%04d fps", zeichner.GibRate()/10*10) })

	var bande, punktezaehler, restzeit Widget = NewFenster(),
		NewMBPunkteAnzeiger(a.billard),
		NewMBRestzeitAnzeiger(a.billard)

	a.spielFenster = NewMBSpieltisch(a.billard)
	a.quizFenster = NewQuizFenster(a.quiz,
		func() { a.billard.ReduziereStrafpunkte(); a.quiz.NaechsteFrage() },
		func() { a.quiz.NaechsteFrage() })

	// Buttonleiste
	a.buttonLeiste = []Widget{
		NewButton("(h)ilfe", a.hilfeAnAus),
		NewButton("(n)eues Spiel", a.Reset),
		NewButton("(m)usik spielen", a.musikAn),
		NewButton("(d)unkel/hell", a.darkmodeAnAus),
		NewButton("(s)chließen", a.quit)}
	...
	a.hilfeFenster = NewTextBox(hilfetext, Regular, int(a.breite/56))

	// GAME OVER
	a.gameOverFenster = NewTextBox(" \n  * GAME OVER *", BoldItalic, int(a.breite/12))
	...
	//frames.SetzeFarben(Fanzeige, Finfos)
	hintergrund.SetzeFarben(Fhintergrund, Ftext)
	a.spielFenster.SetzeFarben(Fbillardtuch, Fdiamanten)
	bande.SetzeFarben(Fbande, Fanzeige)
	punktezaehler.SetzeFarben(Fanzeige, Ftext)
	punktezaehler.SetzeTransparenz(255)
	restzeit.SetzeFarben(Fanzeige, Ftext)
	a.quizFenster.SetzeFarben(Fquiz, Ftext)
	a.hilfeFenster.SetzeFarben(Fquiz, Ftext)
	a.gameOverFenster.SetzeFarben(Fquiz, Ftext)
	for _, b := range a.buttonLeiste {
		b.SetzeFarben(Fhintergrund, Ftext)
	}
...
func (a *bpapp) quit() {
	...
	a.overlay = NewTextOverlay("Bye!", int(a.hoehe/5))
	a.overlay.SetzeKoordinaten(0, 0, a.breite, a.hoehe)
	a.overlay.SetzeFarben(Fanzeige, Ftext)
